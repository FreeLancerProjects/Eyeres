package com.appzone.eyeres.tags;

public class Tags {

    public static final String base_url = "http://iristoresa.com/";
    public static final String IMAGE_URL = base_url+"storage/";
    public static final String session_login = "login";
    public static final String session_logout = "logout";
    public static final int type_add_recent = 1;
    public static final int type_most_sell = 2;

    public static final int new_order = 0;
    public static final int accepted_order = 1;
    public static final int finished_order = 2;
    public static final int refused_order = 3;

    public static final String NEW_ORDER ="new";
    public static final String CURRENT_ORDER ="current";
    public static final String OLD_ORDER ="old";

    public static final int PRODUCT_TYPE_LENSES = 0;
    public static final int PRODUCT_TYPE_ACCESSORIES = 1;

    public static final int ISSIMILAR = 1;
    public static final int NOTSIMILAR = 0;

}
